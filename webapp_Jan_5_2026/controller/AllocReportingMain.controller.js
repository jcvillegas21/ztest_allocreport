sap.ui.define([
...
], (Controller, formatter, SearchField, Filter, FilterOperator, JSONModel, Fragment, Token, Sorter, exportLibrary, Spreadsheet, MessageToast, customVariant, PersonalizableInfo, AllocReportPersTable, TablePersoController, ChartFormatter, ValueHelpDialog, uiTable, uiColumn, Text, Label, TypeString, MessageBox ) => {
    "use strict";
    ...
    return Controller.extend("com.pg.s4.zaatpallocrpt.fiorizaatpallocrpt.controller.AllocReportingMain", {
        ...
        onInit: function() {
         ...
         //  this._generateControlConfigforSFB();
        ...
        onAfterRendering : function(){
          ...
          //  oEventSmartTable.rebindTable();
            // *** MODIFICATION END ***
            // *** ADD THIS CALL ***
             this._applyValueHelpClickBehavior();
    },
        /**
 * Manually adds "click-to-open" behavior to inputs, replacing valueHelpOnly="true".
 */
_applyValueHelpClickBehavior: function() {
    // Array of your MultiInput IDs
    const aInputIds = [
        "AllocReportcharacteristicsSelectionInput_eb", 
        "AllocReportcharacteristicsSelectionInput_ob"
    ];

    aInputIds.forEach(sId => {
        const oControl = this.byId(sId);
        if (oControl) {
            // Remove any existing delegates to prevent duplicates
            oControl.removeEventDelegate(this._oValueHelpDelegate);

            // Create a delegate object
            this._oValueHelpDelegate = {
                ontap: (oEvent) => {
                    // Check if the click target is the input field itself (not the delete icon of a token)
                    if (oEvent.target.classList.contains("sapMInputBaseInner")) {
                        // Stop user from placing cursor/typing
                        oEvent.preventDefault(); 
                        
                        // Fire the value help request manually
                        oControl.fireValueHelpRequest();
                    }
                }
            };

            // Add the delegate to the control
            oControl.addEventDelegate(this._oValueHelpDelegate);
        }
    });
 },
        ...
        // If the "Show Details" filter is active for the order table,
            ...
            if (!bIsOrderBasedTable) {
                if (oFilterData.deletionFlag && oFilterData.deletionFlag !== "All") {
                    if (oFilterData.deletionFlag === "Active") {
                    // aFinalFilters.push(new Filter({ filters: [new Filter("DelFlag", FilterOperator.NE, "X"), new Filter("DelFlag", FilterOperator.EQ, null), new Filter("DelFlag", FilterOperator.EQ, "")], and: false }));
                        aFinalFilters.push(new Filter({ filters: [new Filter("DelFlag", FilterOperator.NE, "X")], and: false }));
                    } else if (oFilterData.deletionFlag === "Deleted") {
                        aFinalFilters.push(new Filter("DelFlag", FilterOperator.EQ, "X"));
                    }
                }
            ...
            if (bIsOrderBasedTable) {
                ...
                // var oSalesDocInput = this.byId("AllocReportSalesDocInput_ob");
                ...
                //     aTokenFilters.push(new Filter("PAllocAssignedOrder", FilterOperator.EQ, oToken.getKey()));
                // });
                
                // // Add to final filters (OR logic between tokens: Doc A OR Doc B)
                // aFinalFilters.push(new Filter({ filters: aTokenFilters, and: false }));
                // }
                // Get the MultiInput control. Use the correct ID for your Order tab.
                var oSalesDocInput = this.byId("AllocReportSalesDocInput_ob"); 
                
                // Safety check for the control and tokens
                if (oSalesDocInput && oSalesDocInput.getTokens().length > 0) {
                    var aSalesDocFilters = [];
                    
                    // Loop through tokens. Since ranges are disabled, every token has a simple key.
                    oSalesDocInput.getTokens().forEach(function(oToken) {
                        var sValue = oToken.getKey();
                        if (sValue) {
                            // Apply a simple Equal filter
                            // We use the OData property you specified: PAllocAssignedOrder
                            aSalesDocFilters.push(new Filter("PAllocAssignedOrder", FilterOperator.EQ, sValue));
                        }
                    });
                    
                    // Combine all filters with OR logic (Doc A OR Doc B)
                    if (aSalesDocFilters.length > 0) {
                        aFinalFilters.push(new Filter({ 
                            filters: aSalesDocFilters, 
                            and: false 
                        }));
                    }
                }
            ...
            // FIX: Only apply Event Reason filter if we are NOT on the Order table
            if (!bIsOrderBasedTable && oFilterData.eventReason) {
                aFinalFilters.push(new Filter("EventId", FilterOperator.EQ, oFilterData.eventReason));
            }
            ...
            {
                 oBindingParams.filters.unshift(new Filter({
        ...
        onTabSelect: function(oEvent) { 
         ...
         //   this.getView().getModel("viewModel").setProperty("/isOrderBasedTab", sSelectedKey === "Order");
          ...
          //  this.loadCustomFilterbarFragmentTabBased(sSelectedKey); 
            ...
            //Check current state
            const bCurrentIsOrder = oViewModel.getProperty("/isOrderBasedTab");
            const bNewIsOrder = (sSelectedKey === "Order");

            if (bCurrentIsOrder !== bNewIsOrder) {
                this._clearAllFilters();
            }

            //Update the view model
            oViewModel.setProperty("/isOrderBasedTab", bNewIsOrder);

            //Set smartfilterbar reference
            this._oSmartFilterBar = bNewIsOrder ? this.byId("orderBased_smartFilterBar") : this.byId("eventBased_smartFilterBar");
            

            // 2. RESET CHARACTERISTICS (Add this line)
             this._resetCharacteristics();
        },

            /**
         * Completely resets the Characteristics logic (Model + UI).
         * Called when switching Main Tabs or clicking "Clear".
         */
        _resetCharacteristics: function() {
            // 1. Reset the Model Data
            var oFilterModel = this.getView().getModel("filterModel");
            oFilterModel.setProperty("/characteristicsData", {
                include: {},
                exclude: {}
            });

            // 2. Clear Visual Tokens from the Main Filter Bar
            var oInputEb = this.byId("AllocReportcharacteristicsSelectionInput_eb");
            var oInputOb = this.byId("AllocReportcharacteristicsSelectionInput_ob");
            if (oInputEb) oInputEb.removeAllTokens();
            if (oInputOb) oInputOb.removeAllTokens();

            // 3. Clear Visual Tokens from ALL Inputs inside the Popup Dialog
            // We list all possible technical field names used in your Fragment
            var aFields = [
                "Vkorg", "Vtweg", "Matnr", "Market", "Ean", 
                "CustHierarchy", "CustGroup" 
                // Add any other field suffixes here if I missed them
            ];

            var aTypes = ["include", "exclude"];
            var aTabs = ["Event", "Order"];

            // Loop through every combination and clear the input if found
            aTypes.forEach(function(sType) {
                aTabs.forEach(function(sTab) {
                    aFields.forEach(function(sField) {
                        // Construct ID: AllocReport + include + Event + Vkorg + Input
                        var sId = "AllocReport" + sType + sTab + sField + "Input";
            ...
            // 4. Trigger Filter Update (Optional: if you want the table to empty immediately)
        ...
        //Clear fragment
        onClearPress: function() { debugger;
    ...
    //       }
        ...
        _attachMultiInputValidators: function() {
            const aMultiInputIds = [
                ...
                "AllocReportSalesDocInput_ob" // <--- ADD THIS NEW ID
            ];
     ...
     // Label Mapping
        var oFieldLabels = {
            ...
            "Ean": "EAN",
            "EAN": "EAN",
            "Kunnr": "Customer Hierarchy",
            "Zzcustgrp": "Customer Group"
        };

    // Helper function to create tokens
    const fnAddTokens = (sTypePrefix, sTypeKey, oData) => {
        if (!oData) { return; }
        
        Object.keys(oData).forEach(sField => {
            if (oData[sField] && oData[sField].length > 0) {
                
                // 1. Get Human Readable Label (default to technical name if missing)
                var sLabel = oFieldLabels[sField] || sField;
                
                // 2. Get Actual Values (joined by comma)
                var sValues = oData[sField].join(", ");

                // 3. Create Token
                var oToken = new Token({
                    key: sField, // Keep unique key
                    text: `${sTypePrefix}. ${sLabel}: ${sValues}` // e.g. "Incl. Sales Organization: DE40, FR40"
                });

                // 4. Attach Technical Data (Critical for the Delete logic to work)
                oToken.data("technicalKey", sField); // Store "Vkorg"
                oToken.data("type", sTypeKey);       // Store "include" or "exclude"
        ...
        fnAddTokens("Incl", "include", oCharacteristics.include);
        fnAddTokens("Excl", "exclude", oCharacteristics.exclude);

        // Update the view
        var bIsOrderBasedTab = this.getView().getModel("viewModel").getProperty("/isOrderBasedTab");
        if (bIsOrderBasedTab) {
            this.byId("AllocReportcharacteristicsSelectionInput_ob").setTokens(aTokens);
        } else {
        ...
        _openValueHelpDialog: function(oData, oSourceControl) {
           ...
           // this._oBasicSearchField = new SearchField();
            ...
            //   // Find SearchField using View's ID scope mechanism
            //     var oSearchField = this.byId("vhSearchField");
            //     // If the SearchField hasn't been created/linked yet, create it now
            //     if (!oSearchField) {
            //         // Create the SearchField instance
            //       oSearchField = new SearchField(oView.createId("vhSearchField"), {
            //             showSearchButton: false, 
            //             placeholder: "Search...", 
            //             width: "100%",
            //             search: this.onValueHelpFilterBarSearch.bind(this) 
            //         });
                    
                   
                    
            //         // Add it as a dependent to ensure cleanup
            //         oFilterBar.addDependent(oSearchField);

            //          // Explicitly link the control instance to the FilterBar
            //         oFilterBar.setBasicSearch(oSearchField);
            //     } else {
            //         // If it already exists (reuse), clear the value
            ...
            //     }]);
                ...
                oTable.addColumn(new sap.ui.table.Column({
                    label: new sap.m.Label({ text: oData.label }),
        ...
        /**
         * Triggered by the "Go" button or the SearchField in the generic Value Help.
         * Filters the table based on Key and Description fields.
         */
        onValueHelpFilterBarSearch: function (oEvent) {
            var sQuery = "";
            // Logic to get the query from the event or the control
            var aSelectionSet = oEvent.getParameter("selectionSet");
            if (aSelectionSet) {
                // Triggered by "Go" button
                var oFilterBar = oEvent.getSource();
                var vBasicSearch = oFilterBar.getBasicSearch();
                var oBasicSearchField = (vBasicSearch instanceof sap.ui.core.Control) ? vBasicSearch : this.byId(vBasicSearch);
                if (oBasicSearchField) {
                     sQuery = oBasicSearchField.getValue();
                }
            } else {
                // Triggered by SearchField Enter
                sQuery = oEvent.getParameter("query") || oEvent.getParameter("newValue");
            }

            // Get Dialog & Model
            var oDialog = this._pValueHelpDialog && typeof this._pValueHelpDialog.then !== 'function' ? this._pValueHelpDialog : this.byId("_IDGenValueHelpDialog");
            if (!oDialog) return;

            var oDialogModel = oDialog.getModel("dialogModel");
            var sKeyField = oDialogModel.getProperty("/key");
            var sDescField = oDialogModel.getProperty("/description");

            // Build Filters
            var aFilters = [];
            if (sQuery && sQuery.length > 0) {
                var aSubFilters = [];
                if (sKeyField) aSubFilters.push(new Filter(sKeyField, FilterOperator.Contains, sQuery));
                if (sDescField) aSubFilters.push(new Filter(sDescField, FilterOperator.Contains, sQuery));
       ...
       createColumnConfiguration: function (oTable) {
        ...
        _generateControlConfigforSFB:function()
        {debugger;
            var listOfFields =["Matnr","MaterialName","Ean","Plant","Vkorg","Vtweg","Market","Zzcustgrp","Zzcustgrpdesc","Kunnr","CustomerName","StartDate","EndDate","Pap","BckConsum","EvtOwner","EventId","EvntComment","DelFlag","ChangeLog","BckCnsptnVal","Meins","ProductHierarchy","ProductCategory","Brand","TimeBucketType","TimeBucketPeriod","ProductAllocationQuantity","ProductAllocationQuantityUnit","ProdAllocAssignedQuantity","ProdAllocAvailableQuantity","Cut_qty","ProdAllocPeriodLoadInPercent","ProductAllocationQuantity_uom","ProdAllocAvailableQuantity_uom","Cut_qty_uom","Base_uom","ProductAllocationQty_allocuom","ProdAllocAssignedQty_allocuom","ProdAllocAvailableQty_allocuom","cut_qty_allocuom","ProductAllocationQuantity_su","ProdAllocAssignedQuantity_su","ProdAllocAvailableQuantity_su","Cut_qty_su","SalesOrder_qty","Delivery_qty","ProdAllocQuantityUnit_su","CvcUuid","Allocuuid","ProdAllocationTimeSeriesUUID","Correl","ProdAllocAssignedQuantity","CvcColor","delivery_uom","EventColor","QtyColor","salesorder_uom","ProdAllocAssignedQuantity_uom"];
            var newControlConfig;
            var objectProperties;
            var sfb_eb, existingControlConfig;
            for(var i=0; i< listOfFields.length;i++)
            {
                objectProperties= {key:listOfFields[i],visible:false}
               //create new control 
               newControlConfig= new sap.ui.comp.smartfilterbar.ControlConfiguration({key:listOfFields[i],visible:false});//"ControlConfig"+"_"+listOfFields[i]+"_"+i, objectProperties
               
               //add this control config to smart filterbar
               sfb_eb = this.byId("eventBased_smartFilterBar");
               existingControlConfig= sfb_eb.getControlConfiguration();
               sfb_eb.addControlConfiguration(newControlConfig);
            }
        ...
        manageCVCButton: function() {
         ...
         // *** START VALIDATION: Check for Deleted Records ***
            for (var i =0; i < aSelectedIndices.length; i++) {
                //Get the binding context for the selected row index
                var oContext = oTable.getContextByIndex(aSelectedIndices[i]);
                if (oContext) {
                    var oData = oContext.getObject();

                    //Check if the Deletion Floag is set to 'X'
                    if (oData.DelFlag === 'X') {
                        MessageBox.error("Deleted records cannot be edited.");
                        return;
                    }
                }
            }
        ...
        onBtnPressAllocMgmt:function()
        {
            this.onNavigationToApps("Zaatppalckpt","display","Zaatpalocrpt-display","");
        },
        onExit:function()
        {
        ...
        _bindUomValueHelp: function() {
            ...
            var oTemplate = new sap.m.ColumnListItem({
                vAlign: "Middle",
                cells: [
                  new sap.m.Text({ text: "{UnitOfMeasure}" }), // AlternativeUnit
                    new sap.m.Text({ text: "{UnitOfMeasure_Text}" }) // Numerator
        ...
        onUomValueHelpSearch: function (oEvent) {
            ...
            var oFilter = new Filter({
                filters: [
                     new Filter("UnitOfMeasure", FilterOperator.Contains, sValue),
                    new Filter("UnitOfMeasure_Text", FilterOperator.Contains, sValue)
        ...
        onUomValueHelpOk: function (oEvent) {
            ...
            if (oSelectedItem) {
                ...
                var sSelectedUom = oSelectedItem.getBindingContext().getProperty("UnitOfMeasure");
        ...
        /**
     * Handles the removal of tokens from the main Characteristics Selection input.
     * Syncs the removal with the filterModel and clears the specific inputs in the Dialog.
     */
    onCharacteristicsTokenUpdate: function(oEvent) { debugger;
    ...
    var oFilterModel = this.getView().getModel("filterModel");
    var oCharData = oFilterModel.getProperty("/characteristicsData");
    
    // Loop through removed tokens
    aRemovedTokens.forEach(function(oToken) {
        // Retrieve the hidden technical data attached to the token
        var sField = oToken.data("technicalKey"); // e.g. "Vkorg"
        var sType = oToken.data("type");          // e.g. "include"

        if (sField && sType) {
            // 1. Update the Model: Remove the data
            if (oCharData[sType] && oCharData[sType][sField]) {
                delete oCharData[sType][sField];
            }

            // 2. Clear Visual Inputs (Force clear BOTH Event and Order inputs)
            this._clearDialogInput(sType, sField, "Event");
            this._clearDialogInput(sType, sField, "Order");
        }
    }.bind(this));

    // Update model and trigger search
    oFilterModel.setProperty("/characteristicsData", oCharData);
    this.onFilterChanged();

    // Refresh the tokens on the main filter bar
    this._updateCharacteristicsTokens();
},

      /**
 * Clears the specific input field inside the popup dialog.
 * Handles SmartMultiInput cleanup (Tokens + Text Value).
 */
_clearDialogInput: function(sType, sTab, sField) {
    // 1. Handle ID naming exceptions (Model Field vs View ID)
    var sFieldIdPart = sField;
    if (sField === "Ean") { sFieldIdPart = "EAN"; } 
    if (sField === "Zzcustgrp") { sFieldIdPart = "CustGroup"; }
    if (sField === "Kunnr") { sFieldIdPart = "CustHierarchy"; }
    
    // 2. Construct the ID
    // Pattern: AllocReport + include + Event + Vkorg + Input
    var sId = "AllocReport" + sType + sTab + sFieldIdPart + "Input";

    // 3. Find the control safely
    var oControl = this.byId(sId);
    if (!oControl) {
        // Fallback for global IDs (common with Fragments)
        oControl = sap.ui.getCore().byId(sId);
    }
    
    // 4. Clear the Control
    if (oControl) {
        // Clear Tokens (The visual bubbles)
        if (oControl.removeAllTokens) {
            oControl.removeAllTokens();
        }
        
        // Clear Text Value (Crucial for SmartMultiInput to look empty)
        if (oControl.setValue) {
            oControl.setValue("");
        }
    } else {
        // If this logs, it means the Fragment hasn't been loaded yet (which is fine),
        // or the ID logic doesn't match your XML.
        // console.log("Debug: Control not found (might not be loaded yet): " + sId);
    }
}
